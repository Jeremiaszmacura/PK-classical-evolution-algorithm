...
def mutationDecisionTreeClassifier(individual):
    ...
    if numberParamer == 0:
        ...
        # splitter
        individual[1] = random.choice(['best', 'random'])
    elif numberParamer == 2:
        # max_depth
        individual[2] = random.randint(2, 10)
    elif numberParamer == 3:
        # max_features
        individual[3] = random.choice(['sqrt', 'log2'])
...
def DecisionTreeClassifierParametersFitness(y, df, numberOfAtributtes, individual):
    split = 5
    ...
    estimator = DecisionTreeClassifier(criterion=individual[0], splitter=individual[1], max_depth=individual[2],
                                       max_features=individual[3], random_state=individual[4])
    resultSum = 0
...
def DecisionTreeClassifierParameters(numberFeatures, icls):
    genome = list()
    ...
    # splitter
    genome.append(random.choice(['best', 'random']))

    # max_depth
    genome.append(random.randint(2, 10))

    # max_features
    genome.append(random.choice(['sqrt', 'log2']))
